    ...
    def on_message(self, *args, **kwargs):
        ...
        if '@kline_' in stream:
            ...
            if t < t2:
                ...
                candle = pd.DataFrame([data], columns='Time Open Close High Low Volume'.split()).astype(
                    {'Time': 'datetime64[ms]'})
...
def calc_volatility(df):
    ...
    # for each value of logReturns, update the model
    # for r in range(len(logReturns)):
    S.step(logReturns)
    ...
    P = bl.Parser(S)
    p_neg_rho = np.array([P('rho < 0.', t=t, silent=True) for t in range(1, 390 - 1)])
    print(p_neg_rho)
    return p_neg_rho
